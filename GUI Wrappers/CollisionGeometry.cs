...
namespace Tool__
{
	public class CollisionGeometry : Wrapper
	{
		...
		private void OnRun(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				processCaller.Arguments = string.Format("collision-geometry {0}", this.SourceDirectory.Field);
				processCaller.Start();

				Process.Start(MainForm.HaloDir + "tool.exe", string.Format("collision-geometry {0}", this.SourceDirectory.Field));
			}
		}
	}
}